...
import {FeatureFlag, User, UserFeatureFlag} from '../utils/sequelize/models';
...
export class FeatureFlagService {
    ...
    async updateUserFeatureFlag(
        ...
        try {
            ...
            const featureFlagToDelete = [featureFlagId];

            if (possibleFeature) {
                newFeatureFlagId = possibleFeature.toJSON().id;
                featureFlagToDelete.push(newFeatureFlagId);
            } else {
            ...
            await UserFeatureFlag.destroy({
                where: {
                    featureFlagId: {[Op.or]: featureFlagToDelete}
                },
            });
